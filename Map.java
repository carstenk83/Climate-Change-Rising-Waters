...
public class Map {
	...
	public Map(double[][] elevations, GridPoint[] waterSources, double waterHeight) 
			throws IllegalArgumentException {
				if (elevations == null || waterSources == null) {
        			throw new IllegalArgumentException("Elevations and/or water sources can't be null.");
    			}
	...
	public double elevation(GridPoint p) throws NotOnMapException {
		if(p.row() < 0 || p.row() >= elevations.length
		|| p.col() < 0 || p.col() >= elevations[0].length){
			throw new NotOnMapException("Grid point is not on the map");
		}
