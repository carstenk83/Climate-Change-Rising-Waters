...
public class Simulation {
	 ...
	 * Simulates flooding using a recursive algorithm
	 * 
	 * @param point on grid being evaluated for flooding
	 */
    public static void simulateRecursiveRec(GridPoint source){
		if(source.row() < 0 || source.row() >= map.numOfRows() 
		|| source.col() < 0 || source.col() >= map.numOfCols() 
		|| underWater(source)){
			return;
		}
		...
		try{
			if(map.belowOrAtHeight(source)){
				...
				simulateRecursiveRec(source.up());
				simulateRecursiveRec(source.down());
				simulateRecursiveRec(source.left());
				simulateRecursiveRec(source.right());
			}
	 ...
	 * @param point on grid being evaluated for flooding
	 */
	public static void simulateStackRec(GridPoint source){
		Stack<GridPoint> temp = new Stack<>();
		temp.push(source);

		while(!temp.isEmpty()){
			GridPoint current = temp.pop();

			if(current.row() < 0 || current.row() >= map.numOfRows() 
			|| current.col() < 0 || current.col() >= map.numOfCols() 
			|| underWater(current)){
				continue;
			}
			...
			try{
				if(map.belowOrAtHeight(current)){
					...
					temp.push(current.up());
					temp.push(current.down());
	 ...
	 * @param point on grid being evaluated for flooding
	 */
	public static void simulateWithQueueRec(GridPoint source){
		Queue<GridPoint> temp = new LinkedList<>();
		temp.add(source);

		while(!temp.isEmpty()){
			GridPoint current = temp.remove();
